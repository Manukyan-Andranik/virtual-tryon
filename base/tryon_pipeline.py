...
class StableDiffusionXLInpaintPipeline(
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            for prompt, tokenizer, text_encoder in zip(prompts, tokenizers, text_encoders):
                ...
                print(f"---------------{617}")
                print(text_input_ids, device)
                print(text_encoder)
                print(next(text_encoder.parameters()).is_cuda)
    ...
    def _encode_vae_image(self, image: torch.Tensor, generator: torch.Generator):
        ...
        image = image.float()
        self.vae.to("cuda", dtype=torch.float32).half()
        print("tryon pipeline 912____________")
        #print(type(self.vae.encode(image.to("cuda"))), type(generator))
        print("__________________")
        print(f"image is device: {image.is_cuda}")
    ...
    def prepare_mask_latents(
        ...
        print(f"------------------prepare_mask_latents device is {device}")
        ...
        print(f"{device}: tryon pipeline 958 masked_image is cuda {masked_image.is_cuda}")
        if masked_image is not None:
            if masked_image_latents is None:
        #        print(f"{device}: tryon pipeline 954 masked_image is cuda {masked_image.is_cuda}")
                masked_image = masked_image.to(device=device, dtype=dtype)
                masked_image_latents = self._encode_vae_image(masked_image.half(), generator=generator)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print("device: tryon pipeline 1622", device)
        ...
        print(f"tryon pipeline 1717 {device}")
        if ip_adapter_image is not None:
            ...
            print("tryon pipeline 1717")
            print(f"image embeds type: {type(image_embeds)}")
            print(f"prompt embes dtype: {prompt_embeds.dtype}")
            print(f"image embeds is cuda: {image_embeds.is_cuda}")
            self.unet = self.unet.to("cuda")
            image_embeds = self.unet.encoder_hid_proj(image_embeds.to("cuda"))#.to(prompt_embeds.dtype)
