...
parallel_parsing_module = DataParallel(parsing_model, device_ids=[0, 1]).module
parallel_opempose_module = DataParallel(openpose_model.preprocessor.body_estimation.model, device_ids=[0, 1]).module
parallel_pipe_module = DataParallel(pipe, device_ids=[0, 1]).module
...
def start_tryon(person_img,garm_img,dress,category, is_checked,is_checked_crop,denoise_steps,seed):
    ...
    if is_checked:
        human_img_tensor = tensor_transfrom(human_img.resize((384,512))).unsqueeze(0).to(device)  # Convert to Tensor and add batch dimension
        keypoints = parallel_opempose_module(human_img_tensor)
        # keypoints = parallel_opempose_module(human_img.resize((384,512)))
        model_parse, _ = parallel_parsing_module(human_img.resize((384,512)))
        print("keypoints___________")
        print(keypoints)
